...
class AetherAPI:
    ...
    def getParameters(self, function):
        ...
        response = requests.get(f"{BASE_URL}/function_params/{function.function_key}/{function.version}", headers=headers)
    ...
    def getParameter(self, function, parameter):
        headers = {'X-API-Key': self.api_key}
        response = requests.get(f"{BASE_URL}/function_param/{function.function_key}/{parameter}/{function.version}", headers=headers)
        if response.status_code != 200:
    ...
    def setParameter(self, function, parameter, value):
        headers = {'X-API-Key': self.api_key}
        response = requests.post(f"{BASE_URL}/function_param/{function.function_key}/{parameter}/{function.version}", headers=headers, json={"value": value})
        if response.status_code != 200:
    ...
    def getCurrentVersion(self, function):
        headers = {'X-API-Key': self.api_key}
        response = requests.get(f"{BASE_URL}/function_data/{function.function_key}", headers=headers)
        if response.status_code != 200:
    ...
    def createCall(self, function, version, inputs, outputs, logs):
        ...
        response = requests.post(f"{BASE_URL}/create_call/{function.function_key}/{version}", headers=headers, json=payload)
        if response.status_code != 200:
    ...
    async def updateCall(self, call):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{BASE_URL}/update_call/{call.call_key}", headers = headers, json=payload) as response:
    ...
    def sync_updateCall(self, call):
        ...
        response = requests.post(f"{BASE_URL}/update_call/{call.call_key}", headers=headers, json=payload)
        if response.status_code != 200:
            raise Exception(f"Error updating call: {response.text}")
        return response.json()
    ...
    async def evaluateCall(self, call):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{BASE_URL}/evaluate_call/{call.function_key}/{call.version}/{call.call_key}", headers=headers, json=payload) as response:
                if response.status_code != 200:
    ...
    def getFunctionData(self, function_key):
        headers = {'X-API-Key': self.api_key}
        response = requests.get(f"{BASE_URL}/function_data/{function_key}", headers=headers)
        if response.status_code != 200:
            raise Exception(f"Error retrieving function data: {response.text}")
        function_data = response.json()
        return function_data
    ...
    async def updateVersionTree(self, function, parent_version, params):
        headers = {'X-API-Key': self.api_key}
        response = requests.post(f"{BASE_URL}/update_version_tree/{function.function_key}/{parent_version}", headers=headers, json={"params": params})
        if response.status_code != 200:
